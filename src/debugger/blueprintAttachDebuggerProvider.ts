...
import { blueprintAttachDebugConfiguration } from "./types";
...
export class blueprintAttachDebuggerProvider extends DebuggerProvider {
    async resolveDebugConfiguration(folder: vscode.WorkspaceFolder | undefined, configuration: blueprintAttachDebugConfiguration, token?: vscode.CancellationToken): Promise<vscode.DebugConfiguration> {
        ...
        configuration.type = "blueprintlua_attach";
    ...
    private async pickPID(processName: string) {
        return new Promise<number>((resolve, reject) => {
            const args = [`"${this.context.extensionPath}/debugger/blueprint/windows/x86/blueprint_tool.exe"`, "list_processes"];
